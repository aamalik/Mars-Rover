/**
 * Instruction mul header class
 */

#ifndef _INSTRUCTION_MUL_HH
#define _INSTRUCTION_MUL_HH
...
class Instruction_mul : public InstructionIF {
private:
	int argc;
...
public:
	...
	/**
     * @brief Public constructor of Instruction_mul class
     * @param  ret   Address in memory to write return value to.
     * @param  args  Vector of arguments needed for instruction.
     */
	Instruction_mul(int* ret, std::vector< std::string > _args);
	~Instruction_mul();

	/**
	* @brief Method to execute the mul instruction of the Brain File
