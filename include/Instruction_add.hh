/**
 * Instruction add header class
 */

#ifndef _INSTRUCTION_ADD_HH
#define _INSTRUCTION_ADD_HH
...
class Instruction_add : public InstructionIF {
private:
	int argc;
...
public:
	...
	/**
     * @brief Public constructor of Instruction_add class
     * @param  ret   Address in memory to write return value to.
     * @param  args  Vector of arguments needed for instruction.
     */
	Instruction_add(int* ret, std::vector< std::string > _args);
	~Instruction_add();

	/**
	* @brief Method to execute the add instruction of the Brain File
