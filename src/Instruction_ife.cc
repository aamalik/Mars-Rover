/**
 * @file Instruction_ife
 * @brief Implementation of the Instruction_ife class
 */ 
...
static const char filename[] = "Instruction_ife";

Instruction_ife::Instruction_ife(int* ret, std::vector< std::string > _args) {
	args = _args;
...
Instruction_ife::~Instruction_ife() {
	args.clear();
}

int Instruction_ife::execute(MemoryHandler* memoryLocation,int* instructionPointer) {
	// Sanity check
