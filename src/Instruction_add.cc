/**
 * @file Instruction_add
 * @brief Implementation of the Instruction_add class
 */ 
...
static const char filename[] = "Instruction_add";

Instruction_add::Instruction_add(int* ret, std::vector< std::string > _args) {
	args = _args;
...
int Instruction_add::execute(MemoryHandler* memoryLocation,int* instructionPointer) {
	...
	switch (t1) {
		...
		case POSITION:
			((Position*)v1)->setX( 
				((Position*)v2)->getX() + ((Position*)v3)->getX());
			((Position*)v1)->setY( 
				((Position*)v2)->getY() + ((Position*)v3)->getY());
			break;
		default: 
			FAIL_ON(true);
	}
