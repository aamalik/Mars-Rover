/**
 * @file Instruction_ieL
 * @brief Implementation of the Instruction_ieL class
 */ 
...
static const char filename[] = "Instruction_ieL";

Instruction_ieL::Instruction_ieL(int* ret, std::vector< std::string > _args) {
	args = _args;
...
Instruction_ieL::~Instruction_ieL() {
	args.clear();
}

int Instruction_ieL::execute(MemoryHandler* memoryLocation,int* instructionPointer) {
	// Sanity check
